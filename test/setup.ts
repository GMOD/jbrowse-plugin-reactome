...
function createTestConfig() {
  return {
    plugins: [
      {
        name: 'Reactome',
        url: `http://localhost:${JBROWSE_PORT}/plugin/jbrowse-plugin-reactome.umd.production.min.js`,
      },
    ],
    assemblies: [
      {
...
export async function createJBrowsePage(browser: Browser) {
  ...
  page.on('pageerror', err => {
    console.log(`[browser page error] ${String(err)}`)
  })
